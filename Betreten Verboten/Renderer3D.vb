...
Public Class Renderer3D
    ...
    Friend Sub LoadContent()
        ...
        SpielfeldVerbindungen = Content.Load(Of Texture2D)("playfield_connections_" & CInt(Game.Map))
            ...
            For i = 0 To 17
                ...
                Select Case i
                    Case PlayFieldPos.Haus1, PlayFieldPos.Haus2, PlayFieldPos.Haus3, PlayFieldPos.Haus4, PlayFieldPos.Home1, PlayFieldPos.Home2, PlayFieldPos.Home3, PlayFieldPos.Home4
                        DrawCircle(loc, sizes.Item2, 25, playcolor(j), 2)
                    Case PlayFieldPos.Feld1
                        DrawCircle(loc, sizes.Item1, 30, playcolor(j), 3)
                        DrawArrow(loc, playcolor(j), j)
                    Case Else
                        DrawCircle(loc, sizes.Item1, 30, Color.White, 3)
                End Select
            Next
        Next
    ...
    Private Sub DrawChr(pos As Vector2, color As Color, basescale As Single, Optional zpos As Integer = 0, Optional scale As Single = 1)
        EffectB.World = figur_model.STLHeader.CenterMatrix * Matrix.CreateTranslation(0, 0, 12) * Matrix.CreateScale(basescale * scale * New Vector3(1, 1, -1)) * Matrix.CreateRotationY(Math.PI) * Matrix.CreateTranslation(-pos.X, -pos.Y, -zpos - AdditionalZPos.Value)
    ...
    Private Sub DrawArrow(vc As Vector2, color As Color, iteration As Integer)
        SpriteBatch.Draw(Pfeil, New Rectangle(vc.X, vc.Y, 35, 35), Nothing, color, MathHelper.PiOver2 * ((iteration / Game.Spielers.Length) * 4 + 3), New Vector2(35, 35) / 2, SpriteEffects.None, 0)
    End Sub
